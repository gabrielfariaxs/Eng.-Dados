...
def fetch_and_store_universities(country_name: str, db_name: str = "universities.db"):
    ...
    if not data:
        print(
            f"Nenhuma instituição encontrada para o país: '{country_name}'. Verifique o nome do país."
        )
        return
    ...
    for uni in data:
        ...
        records_to_insert.append(
            (name, country_api, alpha_two_code, state_province, domains, web_pages)
        )
    ...
    print(
        f"Sucesso! {len(records_to_insert)} instituições salvas na tabela {table_name} no banco '{db_name}'."
    )
...
if __name__ == "__main__":
    pais = input(
        "Digite o nome do país em inglês (ex: Brazil, Canada, United States): "
    )
